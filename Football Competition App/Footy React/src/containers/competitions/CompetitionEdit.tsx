...
const CompetitionEditView = (props: IFormProps<ICompetition>) => {
    ...
    const submitClicked = async (e: Event) => {
        ...
        let response = await BaseService.put<ICompetition>('/Competitions/' + props.values.id, props.values, appState.token!);
        if (response.ok) {
    ...
    return (
        <>
            ...
            { props.values.id !== null && props.values.country ? <>
...
const CompetitionEdit = () => {
    ...
    const [formValues, setFormValues] = useState({} as ICompetition);
    const appState = useContext(AppContext);
    ...
    const handleChange = (target: HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement) => {
        ...
        if (target.id === 'Competition_EndDate') {
            setFormValues({ ...formValues, endDate: target.value === "" ? null : target.value as unknown as Date });
            return;
        }
    ...
    return <CompetitionEditView values={formValues} handleChange={handleChange} />
};
