...
const initialGameLengthValues: IGameLength = {
    halfLength: null,
    extraTimeHalfLength: null
};
...
const GameCreateView = (props: IFormTwoEntityProps<IGame, IGameLength>) => {
    ...
    const loadData = async () => {
        let competitionTeamsResult = await BaseService.getAll<ICompetitionTeam>('/CompetitionTeams/competition?competitionId=' + id, appState.token);
        let gameTypesResult = await BaseService.getAll<IGameType>('/Games/gameTypes', appState.token);

        if (competitionTeamsResult.ok && competitionTeamsResult.data && gameTypesResult.ok && gameTypesResult.data) {
    ...
    const handleValidation = () => {
        ...
        if(props.firstValues.homeScore && props.firstValues.homeScore! as unknown as number % 1 !== 0) {
            setAlertMessage(prevState => ({
                ...prevState,
                homeScore: "Score can not be a float number."
            }));
            formIsValid = false;
        }
        ...
        if(props.firstValues.awayScore && props.firstValues.awayScore! as unknown as number % 1 !== 0) {
            setAlertMessage(prevState => ({
                ...prevState,
                awayScore: "Score can not be a float number."
            }));
            formIsValid = false;
        }
    ...
    const submitClicked = async (e: Event) => {
        ...
        let response = await BaseService.post<IGameAndGameLength>('/Games', {game: props.firstValues, gameLength: props.secondValues}, appState.token!);
        if (response.ok) {
...
const GameCreate = () => {
    ...
    const [gameLengthValues, setGameLengthValues] = useState(initialGameLengthValues);
    const { id } = useParams() as IRouteId;

    useEffect(() => {
        if (id != null) {
            setGameValues({ ...gameValues, competitionId: id });
        }
    }, []); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    return <GameCreateView firstValues={gameValues} secondValues={gameLengthValues} handleChange={handleChange} />
};
